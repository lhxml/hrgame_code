...
public class TwBridge extends UnityPlayerActivity {
	 ...
	 * @return：返回Intent对象
	 */
	private Intent getRoleMsgIntent (final int serverId, final String serverNumber, final String serverName, final String playerId, final String playerName, final int playerLevel)
	{
		Log.d("userinfo", "serverId is " + serverId + " ::: serverNumber is " + serverNumber + " ::: serverName is " + serverName + " ::: playerId is " + playerId + " ::: playerName is " + playerName + " ::: playerLevel is " + playerLevel);
	...
	public void SubmitPlayerInfo(int serverId, String playerId, String playerName, String serverName, int powerValue, int roleCreateTime, int rolePowerModifyTime)
	{
		// 调用某些功能前提交用户信息，越来越多的渠道有此要求
				// 注意，数据上报时不要乱开UI线程
				TwOnlineSDK.sendUserInfo(context, getRoleMsgIntent(serverId, "", serverName, playerId, playerName, powerValue), twCallback);
	}
